...
class AudioFile:
    ...
    def parse_filename(self) -> None:
        pattern = r'(?P<filename>.*?)_?(?P<denoised>denoised)?_?(?P<desilenced>desilenced)?_?(?P<split>split\((?P<split_val>\d+)\))?_?(?P<balanced>balanced)?_?(?P<normalized>normalized\((?P<norm_val>-?\d+(\.\d+)?)\))?_?(?P<mfcc>mfcc\((?P<mfcc_val>\d+)\))?_?(?P<chroma>chroma)?_?(?P<speccontrast>speccontrast)?_?(?P<tonnetz>tonnetz)?\.npy'
        ...
        if match:
            ...
            self.split = bool(match.group("split"))
            self.split_val = int(match.group("split_val"))
    ...
    def generate_filename(self) -> str:
        ...
        if self.split:
            file_name += f'_split({self.split_val})'
        if self.balanced:
            file_name += '_balanced'
        if self.normalized:
